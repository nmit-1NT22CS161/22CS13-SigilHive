...
def _build_prompt(
    ...
    is_select = any(
        kw in query_lower for kw in ["select", "show", "describe", "desc", "explain"]
    )
    is_ddl = any(
        kw in query_lower
        for kw in ["insert", "update", "delete", "create", "alter", "drop"]
    )
...
def _call_gemini_sync(prompt: str) -> str:
    ...
    try:
        ...
        is_select_query = any(
            kw in prompt.lower() for kw in ["select", "show", "describe"]
        )
        ...
        return (
            f"ERROR 2013 (HY000): Lost connection to database server during query - {e}"
        )
    ...
    if raw_response.startswith("ERROR") or raw_response.startswith("Query OK"):
        ...
        if json_data:
            ...
            if "columns" in result and "rows" in result:
                ...
                print(
                    f"[llm_gen] ✓ Valid JSON extracted: {col_count} columns, {row_count} rows"
                )
            else:
