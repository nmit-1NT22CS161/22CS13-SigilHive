...
def _build_logger(name: str) -> logging.Logger:
    ...
    # Only attach a StreamHandler to the top-level "sigilhive" logger.
    # Child loggers (sigilhive.agent, sigilhive.ssh, …) propagate messages
    # up to it automatically — adding handlers to them too causes every
    # message to be printed twice.
...
class SSHAttacker:
    ...
    @staticmethod
    def _sample(
        pool: list, anchors: list, min_extra: int = 2, max_extra: int = 4
    ) -> list:
    ...
    async def recon(self) -> AttackResult:
        cmds = self._sample(
            self.RECON_POOL, self.RECON_ANCHOR, min_extra=2, max_extra=3
        )
    ...
    async def exploit(self, cmds: list[str] | None = None) -> AttackResult:
        ...
        if cmds:
            self.log.debug(
                f"  SSH exploit using {len(cmds)} LLM-generated commands: {cmds}"
            )
        else:
            cmds = self._sample(
                self.EXPLOIT_POOL, self.EXPLOIT_ANCHOR, min_extra=2, max_extra=4
            )
...
class HTTPAttacker:
    ...
    async def recon(self) -> AttackResult:
        paths = self._sample(
            self.RECON_POOL, self.RECON_ANCHOR, min_extra=1, max_extra=3
        )
        ...
        return AttackResult(
            ...
            attack_type="recon",
            success=hits >= 2,
            data_extracted=total,
            suspicion_delta=0.06,
            duration=time.time() - t0,
            responses=responses,
        )
    ...
    async def exploit(self, paths: list[tuple] | None = None) -> AttackResult:
        ...
        if paths:
            self.log.debug(
                f"  HTTP exploit using {len(paths)} LLM-generated paths: {[p for _, p in paths]}"
            )
        else:
            paths = self._sample(
                self.EXPLOIT_POOL, self.EXPLOIT_ANCHOR, min_extra=2, max_extra=4
            ...
            self.log.debug(
                f"  HTTP exploit fallback sample ({len(paths)}): {[p for _, p in paths]}"
            )
        t0 = time.time()
        responses, total, hits = [], 0, 0
        for method, path in paths:
            body = (
                "email=admin@shophub.com&password=admin123" if method == "POST" else ""
            )
...
class DatabaseAttacker:
    ...
    @staticmethod
    def _sample(
        pool: list, anchors: list, min_extra: int = 1, max_extra: int = 3
    ) -> list:
    ...
    async def _read_packet(reader) -> bytes:
        ...
        return (
            b""
            if plen == 0
            else await asyncio.wait_for(reader.readexactly(plen), timeout=5)
        )
    ...
    async def _auth(self, reader, writer) -> bool:
        ...
        body = (
            struct.pack("<I", 0x001FA685)
            + struct.pack("<I", 16777216)
            + struct.pack("B", 33)
            + b"\x00" * 23
            + self.username.encode()
            + b"\x00"
            + struct.pack("B", len(auth_resp))
            + auth_resp
    ...
    async def _query(self, reader, writer, sql: str, seq: int) -> tuple[bool, str, int]:
        ...
        try:
            while True:
                ...
                if body[0] == 0xFF:
                    break
                if body[0] == 0xFE and plen <= 9:
                    break
                if body[0] == 0x00 and plen <= 11:
                    break
    ...
    async def exploit(self, queries: list[str] | None = None) -> AttackResult:
        ...
        if queries:
            self.log.debug(
                f"  DB exploit using {len(queries)} LLM-generated queries: {queries}"
            )
        else:
            queries = self._sample(
                self.EXPLOIT_POOL, self.EXPLOIT_ANCHOR, min_extra=2, max_extra=4
...
def _get_llm() -> ChatGoogleGenerativeAI:
    ...
    if not api_key:
        raise EnvironmentError(
            ...
            "Export your Gemini API key:  export GEMINI_API_KEY='your-key'"
        )
...
def _get_creative_llm() -> ChatGoogleGenerativeAI:
    """Higher-temperature LLM used for exploit command generation so each run varies."""
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        raise EnvironmentError(
...
def _calculate_reward(state: AgentState) -> float:
    ...
    for proto, rk, ek in [
        ...
        if recon and recon.get("success"):
            score += 2.0
        if exploit and exploit.get("success"):
            score += 8.0
        total_data = (recon or {}).get("data_extracted", 0) + (exploit or {}).get(
            "data_extracted", 0
        )
...
def attack_ssh_recon(state: AgentState) -> dict:
    ...
    log.info(
        f"  SSH recon: {'✓' if recon_r.success else '✗'}  suspicion+{recon_r.suspicion_delta:.3f}"
    )
    return {
...
def attack_ssh_exploit(state: AgentState) -> dict:
    ...
    log.info(
        f"  SSH exploit: {'✓' if exploit_r.success else '✗'} suspicion+{exploit_r.suspicion_delta:.3f}"
    )
...
def attack_http_recon(state: AgentState) -> dict:
    ...
    data = state["total_data_bytes"] + recon_r.data_extracted
    log.info(
        f"  HTTP recon: {'✓' if recon_r.success else '✗'}  suspicion+{recon_r.suspicion_delta:.3f}"
    )
    return {
...
def attack_http_exploit(state: AgentState) -> dict:
    ...
    data = state["total_data_bytes"] + exploit_r.data_extracted
    log.info(
        f"  HTTP exploit: {'✓' if exploit_r.success else '✗'} suspicion+{exploit_r.suspicion_delta:.3f}"
    )
    log.info(f"  Cumulative suspicion: {suspicion:.3f}")
    return {
...
def attack_db_recon(state: AgentState) -> dict:
    ...
    data = state["total_data_bytes"] + recon_r.data_extracted
    log.info(
        f"  DB recon: {'✓' if recon_r.success else '✗'}  suspicion+{recon_r.suspicion_delta:.3f}"
    )
    return {
...
def attack_db_exploit(state: AgentState) -> dict:
    ...
    data = state["total_data_bytes"] + exploit_r.data_extracted
    log.info(
        f"  DB exploit: {'✓' if exploit_r.success else '✗'} suspicion+{exploit_r.suspicion_delta:.3f}"
    )
    log.info(f"  Cumulative suspicion: {suspicion:.3f}")
    return {
...
class FileStructureEvolver:
    ...
    _SSH_FILES = [
        ...
        ("~/.vault_token", "s.HONEYTOKEN_VAULT_ROOT_{c:03d}"),
        (
    ...
    def _add_honeytoken_table(self, data: dict, table_name: str) -> str:
        ...
        if entry not in is_rows:
            data["DATABASES"]["information_schema"]["tables"]["tables"]["rows"] = (
                is_rows + [entry]
            )
    ...
    def _dispatch(
        self, change: str, data: dict, successful_protocols: list, cycle: int
    ) -> str:
    ...
    def apply(
        self, approved_changes: list[str], successful_protocols: list[str], cycle: int
    ) -> list[str]:
        """
        ...
        self._write(data)
        self.log.info(
            f"  file_structure.py: {len(applied)} mutation(s) applied → {self.path}"
        )
        return applied
...
def evolve_filesystem(state: AgentState) -> dict:
    ...
    threshold_met = (
        state["overall_suspicion"] >= 0.40
        or state["total_data_bytes"] >= 1024
        or len(successful_protocols) >= 2
    )

    # Build candidate changes grounded in the actual file structure sections
    candidate_changes = []
    if "ssh" in successful_protocols:
        candidate_changes += random.sample(
            EVOLUTION_POOL["ssh"], k=min(2, len(EVOLUTION_POOL["ssh"]))
        )
    if "http" in successful_protocols:
        candidate_changes += random.sample(
            EVOLUTION_POOL["http"], k=min(2, len(EVOLUTION_POOL["http"]))
        )
    if "database" in successful_protocols:
        candidate_changes += random.sample(
            EVOLUTION_POOL["database"], k=min(2, len(EVOLUTION_POOL["database"]))
        )
...
Decide: should the honeypot filesystem evolve?"""
    ...
    try:
        ...
        approved_changes = [
            c for c in decision.get("approved_changes", []) if c in candidate_changes
        ]  # whitelist only
    except json.JSONDecodeError:
        log.warning(
            f"  Gemini returned non-JSON — falling back to threshold. Raw: {raw[:200]}"
        )
    ...
    if should_evolve:
        ...
        fs_path = state.get(
            "file_structure_path", "22CS13-SigilHive\\SigilHive\\file_structure.py"
        )
...
def save_results(state: AgentState) -> dict:
    ...
    log.info(
        f"║  Evolution         : {'✓ TRIGGERED' if state['evolution_triggered'] else '✗ SKIPPED':<45} ║"
    )
...
def _resolve_file_structure_path(given: str) -> str:
    """
    Resolve file_structure_path.  Priority:
      1. Explicit CLI / env value (if non-empty and file exists)
      2. Auto-detect: look for file_structure.py next to this script,
         then in common project sub-paths
      3. Return empty string (evolution will log a warning, not crash)
    """
    if given:
        p = Path(given).resolve()
        if p.exists():
            return str(p)
        root_log.warning(f"Provided file_structure_path does not exist: {given!r}")

    # Auto-detect candidates relative to this script's directory
    script_dir = Path(__file__).resolve().parent
    candidates = [
        script_dir / "file_structure.py",
        script_dir / "honeypots" / "file_structure.py",
        script_dir / "SigilHive" / "file_structure.py",
        script_dir.parent / "file_structure.py",
        script_dir.parent / "honeypots" / "file_structure.py",
        script_dir.parent / "SigilHive" / "file_structure.py",
    ]
    for candidate in candidates:
        if candidate.exists():
            root_log.info(f"Auto-detected file_structure_path: {candidate}")
            return str(candidate)

    root_log.warning(
        "file_structure_path not set and file_structure.py not auto-detected. "
        "Pass --file-structure-path to enable filesystem evolution mutations."
    )
    return ""
...
def run_agent(
    ...
    file_structure_path = _resolve_file_structure_path(file_structure_path)
    graph = build_graph()
    ...
    log.info("╔" + "═" * 70 + "╗")
    log.info(
        "║  SigilHive LangGraph Agent — Gemini-Powered Honeypot Attacker"
        + " " * 8
        + "║"
    )
    log.info("╠" + "═" * 70 + "╣")
    ...
    avg_suspicion = sum(r.get("overall_suspicion", 0) for r in all_results) / len(
        all_results
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of attack episodes to run per scheduled cycle",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--file-structure-path",
        default=os.getenv("FILE_STRUCTURE_PATH", ""),
    ...
    if interval <= 0:
        root_log.info("Running once (--schedule-interval 0).")
        run_agent(**run_kwargs)
        return

    root_log.info(
        f"Scheduler started — attack cycle will repeat every {interval}s "
        f"({interval // 60}m {interval % 60}s). Press Ctrl+C to stop."
    )
    ...
    while True:
        ...
        try:
            ...
            root_log.info("Scheduler interrupted by user — exiting.")
            raise
        except Exception as exc:
            root_log.error(
                f"Run #{run_number} failed with exception: {exc}", exc_info=True
            )
            root_log.warning(f"Will retry in {interval}s.")

        next_run = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(time.time() + interval)
        )
        root_log.info(f"  Next scheduled run at {next_run}  (sleeping {interval}s…)")
        try:
            time.sleep(interval)
        except KeyboardInterrupt:
            root_log.info("Scheduler interrupted during sleep — exiting.")
            return
